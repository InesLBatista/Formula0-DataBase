...
-- Step 1: Check if NomeGP column exists in Resultados
...
-- Step 1b: Populate NomeGP if it's empty
IF EXISTS (SELECT * FROM Resultados WHERE NomeGP IS NULL)
BEGIN
    PRINT 'Populating NomeGP from Sessões...';
    UPDATE r
    SET r.NomeGP = s.NomeGP
    FROM Resultados r
    INNER JOIN Sessões s ON r.NomeSessão = s.NomeSessão
    WHERE r.NomeGP IS NULL;
...
BEGIN
    ...
    UPDATE p
    SET p.NomeGP = s.NomeGP
    FROM Penalizações p
    INNER JOIN Sessões s ON p.NomeSessão = s.NomeSessão
    WHERE p.NomeGP IS NULL;
