...
// Method stubs moved inside class below
namespace ProjetoFBD
{
    public partial class SeasonForm : Form
    {
        ...
        private void LoadSeasonData()
        {
            ...
            string query = "SELECT * FROM vw_Seasons_Overview";
            try
            {
                ...
                if (dgvSeasons != null)
                {
                    ...
                    {
                        var yearCol = dgvSeasons.Columns["Ano"];
                        if (yearCol != null)
                        {
                            yearCol.HeaderText = "Year";
                            yearCol.ReadOnly = true;
                            yearCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                            yearCol.FillWeight = 90;
                        }
                    }
                    if (dgvSeasons.Columns.Contains("NumCorridas"))
                    {
                        var racesCol = dgvSeasons.Columns["NumCorridas"];
                        if (racesCol != null)
                        {
                            racesCol.HeaderText = "Races Count (Auto)";
                            racesCol.ReadOnly = true;
                            racesCol.DefaultCellStyle.Format = "N0";
                            racesCol.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                            racesCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                            racesCol.FillWeight = 90;
                        }
                    }
                    if (dgvSeasons.Columns.Contains("LeaderDriver"))
                    {
                        var driverLeaderCol = dgvSeasons.Columns["LeaderDriver"];
                        if (driverLeaderCol != null)
                        {
                            driverLeaderCol.HeaderText = "Current Driver Leader";
                            driverLeaderCol.ReadOnly = true;
                            driverLeaderCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                            driverLeaderCol.FillWeight = 140;
                        }
                    }
                    if (dgvSeasons.Columns.Contains("LeaderTeam"))
                    {
                        var teamLeaderCol = dgvSeasons.Columns["LeaderTeam"];
                        if (teamLeaderCol != null)
                        {
                            teamLeaderCol.HeaderText = "Current Team Leader";
                            teamLeaderCol.ReadOnly = true;
                            teamLeaderCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                            teamLeaderCol.FillWeight = 130;
                        }
                    }
        ...
        {
            if (chartDriverPodium == null)
                return;
            
            const string driverQuery = "SELECT Abreviação, DriverName, TotalPoints FROM vw_Season_DriverPodium WHERE Year = @Year ORDER BY TotalPoints DESC, Abreviação ASC";
                ...
                using (SqlCommand command = new SqlCommand(driverQuery, connection))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read() && position < 3)
                        {
                            ...
                            chartDriverPodium.Series["Drivers"].Points[pointIndex].ToolTip = $"P{position + 1}: {driverName}\n{totalPoints} points";
        ...
        {
            if (chartTeamPodium == null)
                return;

            const string teamQuery = "SELECT TeamName, TotalPoints FROM vw_Season_TeamPodium WHERE Year = @Year ORDER BY TotalPoints DESC, TeamName ASC";

            try
            {
                using (SqlConnection connection = new SqlConnection(DbConfig.ConnectionString))
                using (SqlCommand command = new SqlCommand(teamQuery, connection))
                {
                    command.Parameters.AddWithValue("@Year", year);
                    connection.Open();
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read() && position < 3)
                        {
                            ...
                            chartTeamPodium.Series["Teams"].Points[pointIndex].ToolTip = $"P{position + 1}: {teamName}\n{totalPoints} points";
        ...
        private int GetGPsCountForYear(int year)
        {
            ...
            {
                using (SqlConnection connection = new SqlConnection(DbConfig.ConnectionString))
                using (SqlCommand command = new SqlCommand("SELECT COUNT(*) FROM Grande_Prémio WHERE Ano_Temporada = @Year", connection))
                {
                    command.Parameters.AddWithValue("@Year", year);
                    connection.Open();
            ...
            {
                MessageBox.Show($"Error calculating races count: {ex.Message}", "Database Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return 0;
            }
        }
        ...
        {
            if (dataAdapter == null || seasonTable == null)
            {
                MessageBox.Show("No season data available to save.", "Nothing to Save",
            ...
            try
            {
                ...
                foreach (DataRow row in seasonTable.Rows)
                {
                    if (row.RowState == DataRowState.Added || row.RowState == DataRowState.Modified)
                    {
                        ...
                        {
                            int rowYear = Convert.ToInt32(row["Ano"]);
                            row["NumCorridas"] = GetGPsCountForYear(rowYear);
                        }
                    }
                }
                ...
                using (SqlConnection connection = new SqlConnection(DbConfig.ConnectionString))
                {
                    dataAdapter.SelectCommand ??= new SqlCommand("SELECT * FROM vw_Seasons_Overview", connection);
            ...
            {
                MessageBox.Show($"Error saving seasons: {ex.Message}", "Save Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        // Move all event handler methods inside the SeasonForm class
    ...
    public class StandingsViewerDialog : Form
    {
        ...
        private void LoadDriverStandings()
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    string query = @"SELECT Position, Driver, Team, TotalPoints, Wins, Podiums FROM vw_Season_DriverStandings WHERE Year = @Year ORDER BY TotalPoints DESC, Wins DESC";
        ...
        private void LoadTeamStandings()
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    string query = @"SELECT Position, Team, TotalPoints, Wins, Podiums FROM vw_Season_TeamStandings WHERE Year = @Year ORDER BY TotalPoints DESC, Wins DESC";
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@Year", year);
                        using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
