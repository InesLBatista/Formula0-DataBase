...
namespace ProjetoFBD
{
    public partial class GPForm : Form
    {
        ...
        private void LoadGPData()
        {
            ...
            // Usar view para dados dos GPs
            string query = "SELECT * FROM vw_GrandPrixDetails ORDER BY Season DESC, DataCorrida ASC";
            try
            {
                dataAdapter = new SqlDataAdapter(query, connectionString);
                gpTable = new DataTable();
                dataAdapter.Fill(gpTable);
                // Comandos de escrita via SPs
                if (userRole == "Staff")
                {
                    SqlCommand insertCmd = new SqlCommand("sp_InsertGrandPrix", new SqlConnection(connectionString));
                    insertCmd.CommandType = CommandType.StoredProcedure;
                    insertCmd.Parameters.Add("@NomeGP", SqlDbType.NVarChar, 100, "NomeGP");
                    insertCmd.Parameters.Add("@DataCorrida", SqlDbType.Date, 0, "DataCorrida");
                    insertCmd.Parameters.Add("@ID_Circuito", SqlDbType.Int, 0, "ID_Circuito");
                    insertCmd.Parameters.Add("@Season", SqlDbType.Int, 0, "Season");
                    dataAdapter.InsertCommand = insertCmd;

                    SqlCommand updateCmd = new SqlCommand("sp_UpdateGrandPrix", new SqlConnection(connectionString));
                    updateCmd.CommandType = CommandType.StoredProcedure;
                    updateCmd.Parameters.Add("@NomeGP", SqlDbType.NVarChar, 100, "NomeGP");
                    updateCmd.Parameters.Add("@DataCorrida", SqlDbType.Date, 0, "DataCorrida");
                    updateCmd.Parameters.Add("@ID_Circuito", SqlDbType.Int, 0, "ID_Circuito");
                    updateCmd.Parameters.Add("@Season", SqlDbType.Int, 0, "Season");
                    dataAdapter.UpdateCommand = updateCmd;

                    SqlCommand deleteCmd = new SqlCommand("sp_DeleteGrandPrix", new SqlConnection(connectionString));
                    deleteCmd.CommandType = CommandType.StoredProcedure;
                    deleteCmd.Parameters.Add("@NomeGP", SqlDbType.NVarChar, 100, "NomeGP");
    ...
    public class AddGPDialog : Form
    {
        ...
        private void LoadCircuits()
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT * FROM vw_Circuits ORDER BY Nome";
                    SqlCommand cmd = new SqlCommand(query, conn);
        ...
        private void LoadSeasons()
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT * FROM vw_Seasons ORDER BY Ano DESC";
                    SqlCommand cmd = new SqlCommand(query, conn);
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
