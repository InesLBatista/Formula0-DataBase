            ...
            if (dataAdapter != null && driverTable != null && userRole == "Staff")
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    try
                    {
                        ...
                        connection.Open();
                        // UpdateCommand
                        SqlCommand updateCmd = new SqlCommand("sp_UpdateDriver", connection)
                        {
                            CommandType = CommandType.StoredProcedure
                        };
                        updateCmd.Parameters.Add("@ID_Piloto", SqlDbType.Int, 0, "ID_Piloto");
                        updateCmd.Parameters.Add("@NumeroPermanente", SqlDbType.Int, 0, "NumeroPermanente");
                        updateCmd.Parameters.Add("@Abreviacao", SqlDbType.NVarChar, 3, "Abreviação");
                        updateCmd.Parameters.Add("@ID_Equipa", SqlDbType.Int, 0, "ID_Equipa");
                        updateCmd.Parameters.Add("@ID_Membro", SqlDbType.Int, 0, "ID_Membro");
                        dataAdapter.UpdateCommand = updateCmd;

                        // InsertCommand
                        SqlCommand insertCmd = new SqlCommand("sp_InsertDriver", connection)
                        {
                            CommandType = CommandType.StoredProcedure
                        };
                        insertCmd.Parameters.Add("@NumeroPermanente", SqlDbType.Int, 0, "NumeroPermanente");
                        insertCmd.Parameters.Add("@Abreviacao", SqlDbType.NVarChar, 3, "Abreviação");
                        insertCmd.Parameters.Add("@ID_Equipa", SqlDbType.Int, 0, "ID_Equipa");
                        insertCmd.Parameters.Add("@ID_Membro", SqlDbType.Int, 0, "ID_Membro");
                        dataAdapter.InsertCommand = insertCmd;

                        // DeleteCommand
                        SqlCommand deleteCmd = new SqlCommand("sp_DeleteDriver", connection)
                        {
                            CommandType = CommandType.StoredProcedure
                        };
                        deleteCmd.Parameters.Add("@ID_Piloto", SqlDbType.Int, 0, "ID_Piloto");
        ...
        private void btnAdd_Click(object? sender, EventArgs e)
        {
            if (driverTable != null && userRole == "Staff")
            {
                try
                {
                    ...
                    // Get Team ID using a selection form (via view)
                    int selectedTeamId = 0;
                    var teams = new System.Collections.Generic.List<(int Id, string Name)>();
                    string teamQuery = "SELECT * FROM vw_Teams ORDER BY Nome";
                    using (SqlConnection connection = new SqlConnection(DbConfig.ConnectionString))
                    {
                        connection.Open();
                        SqlCommand cmd = new SqlCommand(teamQuery, connection);
                    ...
                    if (createNew == DialogResult.Yes)
                    {
                        ...
                        // Birth date is passed to the member insert, not stored in this grid row
                        // Get gender
                    ...
                    else if (createNew == DialogResult.No)
                    {
                        ...
                        // Buscar membros da equipa via view
                        var members = new System.Collections.Generic.List<(int Id, string Display)>();
                        string memberQuery = $"SELECT * FROM vw_TeamMembers WHERE ID_Equipa = {selectedTeamId} ORDER BY Nome";
                        using (SqlConnection connection = new SqlConnection(DbConfig.ConnectionString))
                        {
                            connection.Open();
                            SqlCommand cmd = new SqlCommand(memberQuery, connection);
                            SqlDataReader reader = cmd.ExecuteReader();
                            while (reader.Read())
                            {
